...
) -replace "^{" -replace "}$" | Set-Content "$(Split-Path $Path)\$($Pools.CryptoLight.Name)_CryptoNight_$($Pools.CryptoLight.User)_Nvidia.txt" -Force -ErrorAction SilentlyContinue
