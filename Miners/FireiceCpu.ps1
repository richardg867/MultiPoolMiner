...
) -replace "^{" -replace "}$" | Set-Content "$(Split-Path $Path)\$($Pools.CryptoLight.Name)_CryptoLight_$($Pools.CryptoLight.User)_Cpu.txt" -Force -ErrorAction SilentlyContinue

[PSCustomObject]@{
    Type      = "CPU"
    Path      = $Path
    Arguments = "-c $($Pools.CryptoLight.Name)_CryptoLight_$($Pools.CryptoLight.User)_Cpu.txt --noUAC --noAMD --noNVIDIA"
